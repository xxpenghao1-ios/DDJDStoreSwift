...
class GoodListViewModel:NSObject,OutputRefreshProtocol{
    ...
    init(flag:Int,goodsCategoryId:Int,seachLetterValue:String?,order:String,tag:Int) {
        super.init()
            ...
            .subscribe(onNext: { [weak self] (b) in
                if b {//重新加载数据
                    ...
                    if flag == 1{///
                        ...
                        self?.getGoodsInfoByCategoryForAndroidForStore(b:b, goodsCategoryId:goodsCategoryId, seachLetterValue: seachLetterValue, order: order, tag:tag)
                    }
                }else{///加载下一页
                    self?.currentPage-=1
                    if flag == 1{///
...
extension GoodListViewModel{
    ...
    ///根据3级分类id查询
    private func getGoodsInfoByCategoryForAndroidForStore(b:Bool,goodsCategoryId:Int,seachLetterValue:String?,order:String,tag:Int){
        weak var weakSelf=self
        if weakSelf == nil{
            return
        }
        PHRequest.shared.requestJSONArrModel(target:GoodAPI.queryGoodsInfoByCategoryForAndroidForStore(goodsCategoryId: goodsCategoryId, storeId: store_Id!, priceScreen:nil,order:order, tag:tag, seachLetterValue:seachLetterValue), model:GoodDetailModel.self).subscribe(onNext: { (arr) in
