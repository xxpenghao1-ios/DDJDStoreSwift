...
// MARK: 对外提供的属性
//=======================================================
    var imgSource:ImgSource = ImgSource.LOCAL(name:SLIDE_DEFAULT)  {
        didSet {
            switch imgSource {
            case let .SERVER(url):
                imgView.kf.setImage(with:url, placeholder:UIImage(named:SLIDE_DEFAULT),options:[.transition(ImageTransition.fade(1))])
            case let .LOCAL(name):
    ...
    var imageContentModel:UIViewContentMode = UIViewContentMode.scaleToFill {
        didSet {
