...
class SearchViewController:BaseViewController{
    ...
    private lazy var collection:UICollectionView={
        ...
        flowLayout.headerReferenceSize = CGSize(width:SCREEN_WIDTH, height: 35)
        flowLayout.minimumLineSpacing = 5;//每个相邻layout的上下
        flowLayout.minimumInteritemSpacing = 10;//每个相邻layout的左右
        flowLayout.sectionInset=UIEdgeInsets(top:15,left:15,bottom:15,right:15)
        let _collection=UICollectionView(frame:self.view.bounds, collectionViewLayout:flowLayout)
        _collection.backgroundColor=UIColor.clear
        _collection.emptyDataSetSource=self
        _collection.emptyDataSetDelegate=self
        _collection.register(SearchCollectionViewCell.self, forCellWithReuseIdentifier:"searchId")
        _collection.register(SearchSectionHeader.self, forSupplementaryViewOfKind: UICollectionElementKindSectionHeader, withReuseIdentifier:"HeaderView")
        return _collection
    }()
    ...
    private func setUpNav(){
        //直接创建一个文本框
        let txtSearch=UITextField(frame:CGRect(x:0,y:0, width:SCREEN_WIDTH-100, height:30))
        ...
        searchBtn.rx.tap.asObservable().subscribe { [weak self] (_) in
            self?.vm.addSearchStr.onNext(txtSearch.text)
        ...
        let dataSource = RxCollectionViewSectionedReloadDataSource
            <SectionModel<String,GoodsCategoryModel>>(
                ...
                let sectionView = collectionView.dequeueReusableSupplementaryView(ofKind: kind,
                                                                  withReuseIdentifier: "HeaderView", for:indexPath) as! SearchSectionHeader
                let section=dataSource[indexPath.section]
                sectionView.label.text=section.model
                return sectionView
            })
