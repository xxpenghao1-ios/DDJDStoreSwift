...
extension SpecialViewController:Refreshable{
    ...
    private func bindViewModel(){
        ...
        let dataSources=RxTableViewSectionedReloadDataSource<SectionModel<String,GoodDetailModel>>(configureCell:{ [weak self] (_,table,indexPath,model) in
            ...
            ///点击加入购物车
            cell.btnAddCar.rx.tap.asObservable().subscribe({ (model) in
                ///特价商品区id不一样 重新赋值
                model.goodsbasicinfoId=model.goodsId
                self?.addCarVM=AddCarViewModel(model:model,flag:1)
                self?.addCarVM?.addCarPS.onNext(1)
...
extension SpecialViewController{
    ...
    @objc func timerEvent(){
        for model in vm.specialArrArrModel{//所有数据源每一秒减一
        ...
        for i in 0..<cells.count{
            ...
            if cell != nil{
                let model=vm.specialArrArrModel[cell!.index!]
